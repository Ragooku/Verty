...
static char remote_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "HONOR CHOICE Earbuds X7 Play";
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:{
        ESP_LOGI(GATTC_TAG, "I have connected to the device");
        ...
        // Вывод MAC-адреса подключенного устройства в удобочитаемом формате
        ESP_LOGI(GATTC_TAG, "Connected to MAC address: %02X:%02X:%02X:%02X:%02X:%02X", 
        ...
        // Также сохраняем MAC-адрес для дальнейшего использования
        ESP_LOGI(GATTC_TAG, "Saved remote MAC in profile: "ESP_BD_ADDR_STR"", 
                 ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "Device MAC: "ESP_BD_ADDR_STR"", 
                 ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
        break;
    case ESP_GATTC_DIS_SRVC_CMPL_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "Discovering services for MAC: "ESP_BD_ADDR_STR"", 
                 ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == REMOTE_SERVICE_UUID) {
            ...
            ESP_LOGI(GATTC_TAG, "Service found on device MAC: "ESP_BD_ADDR_STR"", 
                     ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
            get_server = true;
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if(p_data->search_cmpl.searched_service_source == ESP_GATT_SERVICE_FROM_REMOTE_DEVICE) {
            ...
            ESP_LOGI(GATTC_TAG, "Remote device MAC: "ESP_BD_ADDR_STR"", 
                     ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
            ...
            if (count > 0){
                ...
                if (!char_elem_result){
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)){
                        ...
                        ESP_LOGI(GATTC_TAG, "Registering for notifications from device MAC: "ESP_BD_ADDR_STR"", 
                                 ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            ESP_LOGI(GATTC_TAG, "Device MAC for notifications: "ESP_BD_ADDR_STR"", 
                     ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
            uint16_t count = 0;
    ...
    case ESP_GATTC_NOTIFY_EVT:
        if (p_data->notify.is_notify){
            ESP_LOGI(GATTC_TAG, "Notification received from device MAC: "ESP_BD_ADDR_STR"", 
                     ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
        }else{
            ESP_LOGI(GATTC_TAG, "Indication received from device MAC: "ESP_BD_ADDR_STR"", 
                     ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
        }
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "Descriptor write successfully to device MAC: "ESP_BD_ADDR_STR"", 
                 ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "Characteristic write successfully to device MAC: "ESP_BD_ADDR_STR"", 
                 ESP_BD_ADDR_HEX(gl_profile_tab[PROFILE_A_APP_ID].remote_bda));
        break;
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "Disconnected from MAC: %02X:%02X:%02X:%02X:%02X:%02X", 
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (adv_name != NULL) {
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ...
                    ESP_LOGI(GATTC_TAG, "Target device MAC: %02X:%02X:%02X:%02X:%02X:%02X", 
                             scan_result->scan_rst.bda[0], scan_result->scan_rst.bda[1], 
                             scan_result->scan_rst.bda[2], scan_result->scan_rst.bda[3], 
                             scan_result->scan_rst.bda[4], scan_result->scan_rst.bda[5]);
                    ...
                    if (connect == false) {
                        connect = true;
                        ESP_LOGI(GATTC_TAG, "Connect to the remote device with MAC: "ESP_BD_ADDR_STR"", 
        ...
        case ESP_GAP_SEARCH_INQ_CMPL_EVT:
            ESP_LOGI(GATTC_TAG, "Scan complete. No target device found or already connected.");
            break;
        default:
            break;
        }
