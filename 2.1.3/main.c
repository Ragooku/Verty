...
#define ADC_CHANNEL                 ADC_CHANNEL_1     // пример: GPIO32
...
void app_main(void)
{
    ...
    while (1) {
        ...
        if (do_calibration) {
            // Калибровка сырых данных в напряжение
            ...
            printf("ADC%d Channel[%d] Calibrated Voltage: %d mV\n", ADC_UNIT + 1, ADC_CHANNEL, voltage);
                        // Расчет уровня освещенности на основе КАЛИБРОВАННОГО напряжения
            int light_level = (voltage * 100) / 2075;
            printf("Light level: %d %%\n", light_level);
        } else {
            // Если калибровка недоступна, используем приблизительный расчет
            // на основе сырых данных (менее точный)
            voltage = (adc_raw * 2075) / 4095;  // Для 12-битного АЦП
            printf("ADC%d Channel[%d] Uncalibrated Voltage: %d mV\n", ADC_UNIT + 1, ADC_CHANNEL, voltage);
